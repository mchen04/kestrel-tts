...
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "20-distill")); sys.path.insert(0, str(Path(__file__).resolve().parents[2]))
import train2x as TX
from train2x import DSX
from fastkoko.models.vocoder import SFNoiseHead as MaskHead
from model import stft_mag, NFFT as _NF, HOP as _HP
from train import mel_filter
import sys as _s3; from pathlib import Path as _P3; _s3.path.insert(0, str(_P3(__file__).resolve().parents[1] / "111-spec-disc")); from disc2 import Discriminators2 as Discriminators

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--out", default="experiments/104-sf-adversarial/gan")
    ap.add_argument("--resume-gen", default="experiments/103-sf-noise/ckpt")
    ap.add_argument("--steps", type=int, default=23000)
    ...
    gen.load_weights(str(Path(args.resume_gen) / "gen.safetensors"), strict=False)
    dsc = Discriminators()
    step0 = 0
    ...
    def lens_loss_on_fake(fake, real, i):
        of = dsc(fake); orl = dsc(real)
        (sf,ff),(sr_,fr) = of[i], orl[i]
        li = mx.mean((sf-1)**2)
        for a,b in zip(ff,fr):
            li = li + 2.0*mx.mean(mx.abs(a - mx.stop_gradient(b)))
        return li
    ...
    def balance(batch):
        ...
        for i in range(len(LENS_W)):
            g = mx.grad(lambda f: lens_loss_on_fake(f, real, i))(fake)
            mx.eval(g)
            norms.append(float(mx.sqrt(mx.sum(g*g))))
        import numpy as _np
        norms = _np.array(norms) + 1e-12
        w = norms.mean() / norms
        w = _np.clip(w, 0.1, 10.0)
    ...
    def g_loss(batch):
        ...
        of = dsc(fake); orl = dsc(real)
        # cycle 114: DAC-style gradient balancer — per-lens weights set from grad norms every
        # 100 steps (see balance()); frozen between refreshes.
        adv=0.0; fm=0.0
        for w, ((sf,ff),(sr_,fr)) in zip(LENS_W, zip(of,orl)):
            adv = adv + w*mx.mean((sf-1)**2)
            for a,b in zip(ff,fr):
                fm = fm + w*mx.mean(mx.abs(a - mx.stop_gradient(b)))
    ...
    def val_mel(batch):
        ...
        fake = gen.synth(X,F0,N,S,TH, make_noise(X.shape[0], X.shape[1]))[:, m0:m1]
        mfm = stft_mag(fake,1024,256,mel_win)@melfb
        mrm = stft_mag(W[:, m0:m1],1024,256,mel_win)@melfb
        return float(mx.mean(mx.abs(mx.log(mfm+1e-5)-mx.log(mrm+1e-5))))
    ...
    for step in range(step0, args.steps):
        ...
        if not warm:
            if step % 100 == 0:
                wts = balance(ds.batch(args.bs, val=True))
                if step % 500 == 0:
                    print("LENSW " + " ".join(f"{w:.3f}" for w in wts), flush=True)
        ...
        if step % args.log_every == 0:
            vm = val_mel(ds.batch(args.bs, val=True)) if step % 500 == 0 else float("nan")
            print(f"step {step} mel {m:.4f} ema {ema:.4f} val_mel {vm:.4f} {(time.time()-t0)/max(1,step-step0+1):.2f}s/it", flush=True)
