...
import sys as _s; from pathlib import Path as _P; _s.path.insert(0, str(_P(__file__).resolve().parent)); from model_res import ResMaskHead as MaskHead
...
def main():
    ...
    ap.add_argument("--out", default="experiments/56-adv-residual/gan_res")
    ap.add_argument("--res-scale", type=float, default=0.01)
    ap.add_argument("--energy-every", type=int, default=500)
    ap.add_argument("--resume-gen", default="experiments/55-residual-complex/res20k")
    ...
    FRAMES = TX.FRAMES
    ds = DSX()
    gen = MaskHead(dim=args.dim, blocks=args.blocks, res_scale=args.res_scale)
    ...
    def residual_energy(batch):
        X,F0,N,S,TH,W = batch
        h,f0c = gen.trunk(X,F0,N,S)
        M = mx.exp(mx.clip(gen.mask_head(h).astype(mx.float32),-12,8))
        ph = gen.phs_head(h).astype(mx.float32)
        env = mx.exp(mx.clip(gen.nz_head(h).astype(mx.float32),-14,6))
        tre,tim = gen.template(f0c,TH)
        c,sn = mx.cos(ph), mx.sin(ph)
        sre = M*(tre*c-tim*sn); sim = M*(tre*sn+tim*c)
        nr,ni = make_noise(X.shape[0], X.shape[1])
        rre = args.res_scale*gen.res_re(h).astype(mx.float32)
        rim = args.res_scale*gen.res_im(h).astype(mx.float32)
        harm=float(mx.mean(sre**2+sim**2)); noi=float(mx.mean((env*nr)**2+(env*ni)**2)); res=float(mx.mean(rre**2+rim**2))
        t=harm+noi+res
        return 100.0*res/t
    ...
    for step in range(step0, args.steps):
        ...
        if step % args.log_every == 0:
            re_pct = residual_energy(ds.batch(args.bs, val=True)) if step % args.energy_every == 0 else float("nan")
            print(f"step {step} mel {m:.4f} ema {ema:.4f} RESID_ENERGY_PCT {re_pct:.4f} {(time.time()-t0)/max(1,step-step0+1):.2f}s/it", flush=True)
