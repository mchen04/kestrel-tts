...
> # ⛔ WIND-DOWN DIRECTIVE — 2026-08-04 — READ FIRST, OVERRIDES EVERYTHING BELOW
>
> **The owner has ended the Kestrel research program.** This directive supersedes the "never
> stop" objective, §5's loop, and every open question in §7. It was issued after cycle 113
> landed (UTMOS 4.1803 — the program's best result), and cycle 113 is the final cycle.
>
> **Do not start cycle 114. Do not run any experiment. Do not train anything.**
>
> Run the following wind-down tasks once, in order, then stop permanently.
>
> **1. Stop the loop.** Call `ScheduleWakeup` with `stop: true` and kill any persistent
> `Monitor` tasks (e.g. the `train_gan.log` watchers). Confirm no training process is running
> (`ps aux | grep train_gan`). Do this FIRST so nothing re-enters the loop mid-wind-down.
>
> **2. Update Hugging Face** (`mchen04/kestrel-tts`, via `hf/push_to_hf.py`). Push the cycle-113
> default checkpoint `weights/kestrel_sf_lw58k` alongside the existing four, and rewrite the Hub
> model card to the final measured state: UTMOS 4.1803, DNSMOS 3.2374, NISQA 4.7199, eval WER
> 5.46 %, robustness WER 16.81 %, `student-fast` 0.271 s / ×622. Keep the honest-limits section —
> the texture gap (MCD 13.89 vs the 3.98 control bar) stays stated plainly. If HF auth is missing,
> stop and report it rather than guessing at credentials.
>
> **3. Clean regenerable binaries** (working tree is ~36 GB). Delete from every `experiments/*/`:
> `gan/gen_*.safetensors`, `gan/dsc.safetensors`, `gan/gen.safetensors`, `render*/`, `rob_*/`,
> and loose `*.wav`. **Keep** every `PLAN.md`, `RESULT.md`, `LEDGER.md`, metrics/eval `*.json`,
> and every training script. **Never touch** `weights/`, `eval/`, `bench/`, `baseline/`,
> `fastkoko/`, or `docs/`. Before deleting anything, verify `weights/` is intact and all three
> presets still load and synthesise finite audio — those weights are the only irreplaceable
> binaries in the repo.
>
> **4. Update documentation for a finished project.** `README.md` (final frontier table with the
> cycle-113 numbers), `docs/MODEL_CARD.md`, and `docs/README.md`. Rewrite the "Ongoing work"
> section: the loop is closed after 113 cycles, and this is the final state. Add a short closing
> summary to `experiments/LEDGER.md` — where each axis finished, and the open questions left
> unanswered — but **do not edit any existing ledger row**; it stays append-only. Archive
> `RESEARCH.md` itself to `docs/history/` and leave a stub noting the program is closed.
>
> **5. Commit, rewrite history, push.** Commit the wind-down. Then use `git filter-repo` to strip
> `experiments/*/**/*.safetensors`, `experiments/*/render*/`, and `*.wav` from all 65 unpushed
> commits — `origin/main` is still at the Jul 29 initial commit and nothing downstream depends on
> this history, so the rewrite is safe. Preserve `weights/` (~53 MB) and every `.md`/`.json`.
> Expected result: ~9.3 GB → well under 1 GB. Add the deleted patterns to `.gitignore`. Verify the
> rewritten tree still has every `RESULT.md` and `weights/` file, then `git push origin main`
> (force-with-lease as required by the rewrite).
>
> **6. Report and stop.** Post a final summary: what shipped, the HF URL, the repo size before and
> after, and anything that failed. Then stop — no wakeup, no further cycles. If any step fails,
